...
import { Code, Users, BookOpen, Search, Calendar, Filter } from 'lucide-react';
...
export function DevBlogs() {
  ...
  const authors = Array.from(new Set(blogArticles.map(a => a.author).filter(Boolean)));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
          ...
          <div className="mt-4 pt-4 border-t border-slate-700">
            <div className="text-sm text-slate-400">
              ...
              <span className="font-medium text-slate-100">{blogArticles.length}</span> articles
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`grid gap-6 ${selectedArticle ? 'grid-cols-1 xl:grid-cols-2' : 'grid-cols-1 xl:grid-cols-4'}`}>
            ...
            <div className="p-6">
              <div className="space-y-4">
                {paginatedArticles.map((article) => (
                  ...
                  >
                    <Code className="w-5 h-5 text-purple-500 mt-1 flex-shrink-0" />
        ...
        {/* Blog Stats - shows when no article is selected */}
        {!selectedArticle && (
          <div className="xl:col-span-1 space-y-6">
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
            <div className="flex items-center gap-3 mb-4">
              <Code className="w-6 h-6 text-purple-400" />
              <h3 className="text-lg font-semibold text-slate-100">Dev Tutorials</h3>
            </div>
            <p className="text-3xl font-bold text-slate-100">
              {blogArticles.filter(a => a.tags.some(tag => tag.toLowerCase().includes('tutorial'))).length}
            </p>
            <p className="text-slate-400 text-sm">Tutorial articles</p>
          </div>

          <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
            <div className="flex items-center gap-3 mb-4">
              <Users className="w-6 h-6 text-green-400" />
              <h3 className="text-lg font-semibold text-slate-100">Authors</h3>
            </div>
            <p className="text-3xl font-bold text-slate-100">
              {authors.length}
            </p>
            <p className="text-slate-400 text-sm">Contributing developers</p>
          </div>
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
            ...
            <p className="text-3xl font-bold text-slate-100">
              {Math.round(blogArticles.reduce((sum, a) => sum + a.readingTime, 0) / blogArticles.length || 0)}
            </p>
            <p className="text-slate-400 text-sm">Avg minutes per post</p>
          </div>
          </div>
        )}
      </div>
