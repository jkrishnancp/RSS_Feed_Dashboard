...
import { Shield, AlertTriangle, TrendingUp, Lock, Search, Calendar, Filter } from 'lucide-react';
...
export function CyberSecurity() {
  ...
  const filteredArticles = cyberArticles.filter(article => {
    const now = new Date();
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
          ...
          <div className="mt-4 pt-4 border-t border-slate-700">
            <div className="text-sm text-slate-400">
              ...
              <span className="font-medium text-slate-100">{cyberArticles.length}</span> articles
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`grid gap-6 ${selectedArticle ? 'grid-cols-1 xl:grid-cols-2' : 'grid-cols-1 xl:grid-cols-4'}`}>
            ...
            <div className="p-6">
              <div className="space-y-4">
                {paginatedArticles.map((article) => (
                  ...
                  >
                    <Lock className="w-5 h-5 text-orange-500 mt-1 flex-shrink-0" />
        ...
        {/* Threat Stats - shows when no article is selected */}
        {!selectedArticle && (
